東方深秘録 対戦記録自動集計プログラム（仮）報告ツール v0.00
http://tenco.info/
2014/12/29

○概要
このソフトウェアは、Tenco! サーバーに東方深秘録 対戦記録自動集計プログラムの
対戦結果データを報告するコマンドラインアプリケーションです。
Tenco! の詳細は、上記URLからご覧ください。

○東方深秘録 対戦記録自動集計プログラムについて

さくらゆきかぜさん（@s_yukikaze）作成の対戦結果記録ツールです。
心綺楼でいう綺録帖、非想天則でいう天則観にあたるツールになります。
...
東方深秘録 対戦記録自動集計プログラムのフォルダ内においてください。
フォルダ名は自由に変えていただいてかまいません。

２．spr_report.exe を実行（初回）
...
デフォルトでは、全ての東方深秘録 対戦記録自動集計プログラムのプロファイルのデータを送信します。
対戦結果の報告が終わると、ウィンドウが勝手に閉じます。

３．以後の報告
spr_report.exe を立ち上げると、前回報告分以降の対戦結果を
自動的にサーバーに送ってくれます。
...
一番下の database: -> file_path: の部分で、東方深秘録 対戦記録自動集計プログラムのデータベースのパスを指定できます。
...
database: 
  file_path: ../*.db   # 東方深秘録 対戦記録自動集計プログラムのデータベースファイルパス
----
データベースファイルパスは、固定名・ワイルドカードを使った指定のどちらでもかまいません。
固定名とすると、１つの東方深秘録 対戦記録自動集計プログラムプロファイルのみを報告することができます。
...
○謝辞
プレイヤーの皆様
東方深秘録 対戦記録自動集計プログラムを制作したさくらゆきかぜさん（@s_yukikaze）
Tenco!ユーザーの皆様
黄昏フロンティアの皆様
