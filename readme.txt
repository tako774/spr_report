...
本ツールは、辺獄録に記録した対戦結果を報告する仕組みですので、先にこちらの導入をお願い致します。
入手先は、東方 Tools Wiki の下記ページに記載されています。
http://mint.s150.xrea.com/?%E8%BE%BA%E7%8D%84%E9%8C%B2

○インストール方法・使い方

１．インストール手順

圧縮ファイルを展開してできたフォルダ spr_report_v*** を「フォルダごと」、
辺獄録のフォルダ内においてください。
...
デフォルトでは、全ての辺獄録のプロファイルのデータを送信します。
...
一番下の database: -> file_path: の部分で、辺獄録のデータベースのパスを指定できます。
...
database: 
  file_path: ../*.db   # 辺獄録のデータベースファイルパス
----
データベースファイルパスは、固定名・ワイルドカードを使った指定のどちらでもかまいません。
固定名とすると、１つの辺獄録プロファイルのみを報告することができます。
...
○謝辞
プレイヤーの皆様
辺獄録を制作したさくらゆきかぜさん（@sakra_yukikaze）
Tenco!ユーザーの皆様
黄昏フロンティアの皆様
